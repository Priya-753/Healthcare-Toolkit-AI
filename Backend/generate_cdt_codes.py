...
def get_cdt_icd_codes(transcript, soap):
    ...
    messages=[
        ...
        "content": [
            {
            "type": "text",
            "text": f"""You're an expert in diagnosing ICD and CDT codes from given SOAP notes and transcripts. Given a transcript and SOAP notes for a dental appointment, you will generate ICD and CDT codes only for the treatments, diagnoses, and procedures discussed during the current appointment. Ignore any reference to previous treatments, such as past fillings, cleanups, or other historical work. you will also keep in mind to mention the quadrant, teeth type(molar, premolar, canine), teeth number since all of this data is required in the next step of where your output will be used which is to generate pre-authorisation and insurance claims. response will be in the following json format - {{"procAndBillingCodes": {{<array of ICD diagnosis and corresponding CDT procedure where both will be objects like icd: <icdObject>, cdt: <cdtObject>>}} }}
